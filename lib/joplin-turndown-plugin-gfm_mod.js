var turndownPluginGfm = (function(exports) {
    ...
    rules.table = {
        ...
        replacement: function(content, node) {
            ...
            allRows.forEach((line, rowNumber) => {
                ...
                var newLineOfCells = [];
                var linesInCell = [];
                if ($("input[name='process_mode']:checked").val() == "text_table") {
            ...
            allCells.forEach(function(line, index) {
                ...
                line.forEach(function(cell, idx) {
                    ...
                    if (cell == "<td/>") {
                        ...
                        $("input[name='process_mode']:checked").val() == "markdown" ||
                        $("input[name='process_mode']:checked").val() == "text_table"
                    ) {
                        pad = "  ";
                        divider = "|";
                    }
                    // We don't want initial divider in case of simpler_text
                    allCells[index][idx] = divider + cell.padEnd(maxWidth[idx], pad);

                    if ($("input[name='process_mode']:checked").val() == "simpler_text") {
                        pad = "  ";
                        divider = " / ";
                    }
    ...
    function handleColSpan(content, node, emptyChar) {
        ...
        for (let i = 1; i < colspan; i++) {
            if ($("input[name='process_mode']:checked").val() == "markdown") {
                content += " | " + emptyChar.repeat(3);
            } else if (
                $("input[name='process_mode']:checked").val() == "simpler_text"
            ) {
                content += " / ";
            } else {
                content += "|";
